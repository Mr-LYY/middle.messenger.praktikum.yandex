...
(function (global, factory) {
    ...
    Color.prototype = Object.assign(new Node(), {
        ...
        toCSS(context, doNotCompress) {
            ...
            switch (colorFunction) {
                case 'rgba':
                    args = this.rgb
                        .map(c => clamp$1(Math.round(c), 255))
    ...
    ***************************************************************************** */
    ...
    function copy(target, options) {
        ...
        if (isArray(target)) {
            return target.map(i => copy(i, options));
        }
        ...
        return __spreadArrays(props, symbols).reduce((carry, key) => {
    ...
    Selector.prototype = Object.assign(new Node(), {
        ...
        eval(context) {
            ...
            extendList =
                extendList && extendList.map(extend => extend.eval(context));
    ...
    const debugInfo = /** @class */ (function () {
            ...
            function addReplacementIntoPath(
                ...
                if (addPath.length > 0) {
                    ...
                    newJoinedSelector.elements =
                        newJoinedSelector.elements.concat(
    ...
    ***************************************************************************** */
    ...
    const functionCaller = /** @class */ (function () {
        ...
        functionCaller.prototype.call = function (args) {
            ...
            const { evalArgs } = this.func;
            if (evalArgs !== false) {
                args = args.map(a => a.eval(_this.context));
            }
            const commentFilter = function (item) {
            ...
            if (evalArgs === false) {
                return this.func.apply(
                    this,
                    __spreadArray([this.context], args),
                );
            }
                ...
                if (v) {
                    if (v.important) {
                        const importantScope =
                            context.importantScope[
                                context.importantScope.length - 1
                            ];
        ...
        eval(context) {
            ...
            function calcDefGroup(mixin, mixinPath) {
                ...
                for (f = 0; f < 2; f++) {
                    ...
                    for (
                        p = 0;
                        p < mixinPath.length && conditionResult[f];
                        p++
                    ) {
            ...
            for (i = 0; i < context.frames.length; i++) {
                if (
                    ...
                    for (m = 0; m < mixins.length; m++) {
                        ...
                        for (f = 0; f < context.frames.length; f++) {
                            if (
                                ...
                                mixin ===
                                    (context.frames[f].originalRuleset ||
                                        context.frames[f])
                            ) {
                                isRecursive = true;
                                break;
                    ...
                    for (m = 0; m < candidates.length; m++) {
                        ...
                        if (
                            candidate === defNone ||
                            candidate === defaultResult
                        ) {
                            try {
                                mixin = candidates[m].mixin;
                                if (!(mixin instanceof Definition)) {
                                    originalRuleset =
                                        mixin.originalRuleset || mixin;
    ...
    ImportVisitor.prototype = {
        ...
        onImported(importNode, context, e, root, importedAtRoot, fullPath) {
            ...
            const inlineCSS = importNode.options.inline;
            const { isPlugin } = importNode.options;
    ...
    const ProcessExtendsVisitor = /** @class */ (function () {
        ...
        ProcessExtendsVisitor.prototype.checkExtendsForNonMatched = function (
            ...
            extendList
                .filter(
                    extend =>
                        !extend.hasFoundMatches &&
                        extend.parent_ids.length == 1,
                )
                .forEach(extend => {
        ...
        ProcessExtendsVisitor.prototype.doExtendChaining = function (
            ...
            for (
                ...
                for (
                    ...
                    if (
                        extend.parent_ids.indexOf(targetExtend.object_id) >= 0
                    ) {
                        continue;
                    }
        ...
        ProcessExtendsVisitor.prototype.visitRuleset = function (
            ...
            for (
                ...
                for (
                    ...
                    const { extendList } =
                        selectorPath[selectorPath.length - 1];
                        ...
                        if (
                            ...
                            (potentialMatch.matched > 0 &&
                                needleElements[potentialMatch.matched]
                                    .combinator.value !== targetCombinator)
                        ) {
                            potentialMatch = null;
                        } else {
            ...
            groupsArr.forEach(group => {
                if (group.length > 0) {
                    ...
                    group.forEach(rule => {
                        ...
                        result_1.important =
                            result_1.important || rule.important;
                    });
    ...
    const Parser = function Parser(context, imports, fileInfo) {
        ...
        function parseNode(str, parseList, currentIndex, fileInfo, callback) {
            ...
            try {
                parser.start(str, false, (msg, index) => {
                    callback({
                        message: msg,
            ...
            parsers: (parsers = {
                ...
                entities: {
                    ...
                    keyword() {
                        ...
                        if (k) {
                            return (
                                tree.Color.fromKeyword(k) || new tree.Keyword(k)
                            );
                        }
                    },
                    //
                ...
                mixin: {
                    ...
                    call(inValue, getLookup) {
                        ...
                        if (elements) {
                            ...
                            if (inValue || parsers.end()) {
                                parserInput.forget();
                                const mixin = new tree.mixin.Call(
                                    elements,
                                    args,
                                    index,
                                    fileInfo,
                ...
                combinator() {
                    ...
                    if (c === '/') {
                        parserInput.save();
                        const slashedCombinator =
                            parserInput.$re(/^\/[a-z]+\//i);
                ...
                atrule() {
                    ...
                    if (name.charAt(1) == '-' && name.indexOf('-', 2) > 0) {
                        nonVendorSpecificName = `@${name.slice(
                            name.indexOf('-', 2) + 1,
                        )}`;
                    }
                    ...
                    if (
                        rules ||
                        (!hasBlock && value && parserInput.$char(';'))
                    ) {
                        parserInput.forget();
                ...
                parenthesisCondition(needsParens) {
    ...
    for (const f$1 in colorBlendModeFunctions) {
        if (colorBlendModeFunctions.hasOwnProperty(f$1)) {
            colorBlend[f$1] = colorBlend.bind(
                null,
                colorBlendModeFunctions[f$1],
            );
        }
    }
            ...
            SourceMapBuilder.prototype.toCSS = function (
                ...
                if (this.options.sourceMapInputFilename) {
                    this.sourceMapInputFilename =
                        sourceMapOutput.normalizeFilename(
                ...
                const loadFileCallback = function (loadedFile) {
                    ...
                    if (importOptions.isPlugin) {
                        ...
                        if (
                            ...
                            !importManager.files[resolvedFilename].options
                                .multiple &&
        ...
        var parse = function (input, options, callback) {
            ...
            if (!callback) {
                ...
                return new Promise((resolve, reject) => {
                    parse.call(self_1, input, options, (err, output) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(output);
            ...
            const pluginManager_1 = new PluginManagerFactory(
                this,
    ...
    function Render(environment, ParseTree, ImportManager) {
        var render = function (input, options, callback) {
            ...
            if (!callback) {
                const self_1 = this;
                return new Promise((resolve, reject) => {
                    render.call(self_1, input, options, (err, output) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(output);
    ...
    const ErrorReporting = function (window, less, options) {
        ...
        function errorConsole(e, rootHref) {
            ...
            const filename = e.filename || rootHref;
            const errors = [];
            let content = `${e.type || 'Syntax'}Error: ${
                e.message || 'There is an error in your .less file'
            } in ${filename}`;
            const errorline = function (e, i, classname) {
                if (e.extract[i] !== undefined) {
                ...
                if (remainingSheets === 0) {
                    ...
                    loadStyleSheets(
                        (e, css, _, sheet, webInfo) => {
                            ...
                            browser.createCSS(window.document, css, sheet);
                            less.logger.info(
                                `CSS for ${sheet.href} generated in ${
                                    new Date() - endTime
                                }ms`,
                            );
